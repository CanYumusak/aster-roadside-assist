...
**Voice reliability.** The current OpenAI Realtime setup is fast to prototype and convincing for a demo, but background noise, VAD/noise tuning, interruption handling, tone consistency, and call-ending behavior need work. If the client needs stronger brand voice, pronunciation, and emotional tone control, we should evaluate a specialist voice layer such as ElevenLabs. 
...
The improvement loop is eval-driven. Offline evals replay historical cases against classifiers, prompts, state-machine changes, and location resolution before release. Online evals run in shadow mode with QA labels for safety, identity, location, incident, coverage, escalation, tone, and latency. Changes promote only when predefined thresholds improve. If voice quality becomes the bottleneck, we would evaluate ElevenLabs for branded voice control or our own STT + LLM + TTS stack for maximum turn-taking, telephony, and provider control.
